// Step 5: Asynchronous event delivery (simulate concurrency)
...
class EventBus {
  ...
  publish(event: Event) {
    ...
    for (const handler of handlers) {
      // simulate async delivery (non-blocking)
      setTimeout(() => handler(event), Math.random() * 500);
    }
  }
}
...
bus.subscribe("OrderPlaced", (event) => {
  ...
  // simulate latency in billing itself
  setTimeout(() => {
